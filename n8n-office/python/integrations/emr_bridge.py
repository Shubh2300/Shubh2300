...
def _audit(
    ...
    try:
        conn.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?)",
            (ts, source, op, key, outcome, detail),
        )
        conn.commit()
    finally:
        conn.close()
    ...
    try:
        ...
        with AUDIT_LOG.open("a", encoding="utf-8") as fh:
            fh.write(
                f"[{ts}] source={source or '-'} op={op} key={key} "
...
async def svigg_lookup_patient(name: str) -> dict:
    """Direct Svigg lookup via the new Playwright scraper. Returns structured patient data."""
    from svigg_scraper import SviggScraper

    # Check cache first
    cached = _cache_get("svigg_live", "lookup", name)
    if cached:
        _audit("svigg_live", "lookup", name, "cache_hit")
        return cached
    ...
    try:
        ...
        if not logged_in:
            _audit("svigg_live", "lookup", name, "session_expired", "Svigg login failed")
            raise EMRSessionExpired("Svigg login failed — check WEBEDOCTOR_USER/WEBEDOCTOR_PASS in .env")

        # Split name into last, first if comma-separated
        parts = [p.strip() for p in name.split(",", 1)]
        last_name = parts[0]
        first_name = parts[1] if len(parts) > 1 else ""

        # If space-separated (First Last), swap
        ...
        _cache_put("svigg_live", "lookup", name, results)
        _audit("svigg_live", "lookup", name, "ok", f"{len(results)} results")
        return {"source": "svigg_live", "results": results}
    except EMRSessionExpired:
        raise
    except Exception as e:
        _audit("svigg_live", "lookup", name, "error", str(e))
        return {"source": "svigg_live", "error": str(e)}
    finally:
        await scraper.stop()
